...
class FontBar(tk.Tk):
    def __init__(self, parent,teststr):
        ...
        self.textwidget= st.ScrolledText(self.parent, height=55, width=100)
        self.textwidget.grid(row=10,column=0)
        self.textwidget.insert("1.0", teststr)
...
##            self.textwidget_change = text_change
##            words = len(self.textwidget.get(1.0, "end-1c").split())
##            characters = len(self.textwidget.get(1.0, "end-1c"))
##            self.status.config(textwidget=f"Characters : {characters} Words : {words}")
##            self.textwidget.edit_modified(False)
##
##    self.textwidget.bind("<<Modified>>", changed)
##    
